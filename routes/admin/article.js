...
router.get('/',async (ctx)=>{
    ...
    // console.log(count);
    var result=await DB.findPage('article',{},{},{
        page:page,
        pageSize:pageSize,
        sort:{
            'add_time':-1 //按时间降序，即新增的内容放在最前头
        }
    });
    //  console.log(result);
    await ctx.render('admin/article/index',{
        list:result,
        page:page,
        totalPages:Math.ceil(count/pageSize)//Math.ceil:向上取整
    });
});
...
router.post('/doAdd',myupload.single('pic'), async (ctx)=>{//不用post 则会提示 method not allow
    ...
    let content=ctx.req.body.maincontent ||'';
    let img_url=ctx.req.file? ctx.req.file.path.substr(9):'';
